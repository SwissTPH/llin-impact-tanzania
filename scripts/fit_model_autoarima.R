...
fit_auto_arima_with_exog = function(ts_var, exog_vars, merged_df) {
  ...
  model = auto.arima(ts_var, xreg = xreg,
                     stepwise=FALSE, trace=FALSE,
                     method ="ML", approximation=FALSE, test = "adf")
...
best_model_with_ITN = auto.arima(ts_incidence, xreg = xreg_with_ITN, stepwise=FALSE, 
                                 trace = FALSE, method ="ML",approximation=FALSE,test = "adf")
